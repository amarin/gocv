...
ifeq ($(OS),Windows_NT)
	...
	HOST_OS="darwin"
	TARGET_OS="darwin"
	CMAKE_BUILD_PARALLEL_LEVEL=$(shell sysctl -n hw.logicalcpu)
else ifeq ($(shell uname), Linux)
	HOST_OS="linux"
	TARGET_OS="linux"
	CMAKE_BUILD_PARALLEL_LEVEL=$(shell nproc --all)
endif

# Define build path
BUILD_PATH := ${TMP_DIR}opencv/opencv-${OPENCV_VERSION}/build

# reduce mistyping defining OPENCV_EXTRA_MODULES_PATH once
OPENCV_EXTRA_MODULES_PATH=$(TMP_DIR)opencv/opencv_contrib-$(OPENCV_VERSION)/modules
...
build_openvino_package:
	...
	sudo $(MAKE) -j $(CMAKE_BUILD_PARALLEL_LEVEL)
	sudo touch VERSION
...
build_openvino:
	...
	cd build
	rm -rf *
	cmake -D CMAKE_BUILD_TYPE=RELEASE -D CMAKE_INSTALL_PREFIX=/usr/local -D BUILD_SHARED_LIBS=${BUILD_SHARED_LIBS} -D ENABLE_CXX11=ON -D OPENCV_EXTRA_MODULES_PATH=$(OPENCV_EXTRA_MODULES_PATH) -D WITH_INF_ENGINE=ON -D InferenceEngine_DIR=/usr/local/dldt/inference-engine/build -D BUILD_DOCS=OFF -D BUILD_EXAMPLES=OFF -D BUILD_TESTS=OFF -D BUILD_PERF_TESTS=OFF -D BUILD_opencv_java=NO -D BUILD_opencv_python=NO -D BUILD_opencv_python2=NO -D BUILD_opencv_python3=NO -D WITH_JASPER=OFF -D WITH_TBB=ON -DOPENCV_GENERATE_PKGCONFIG=ON -DOPENCV_ENABLE_NONFREE=ON ..
	$(MAKE) -j $(CMAKE_BUILD_PARALLEL_LEVEL)
	$(MAKE) preinstall
	cd -
...
build_all:
	...
	cd build
	rm -rf *
	cmake -j $(CMAKE_BUILD_PARALLEL_LEVEL) -D CMAKE_BUILD_TYPE=RELEASE -D CMAKE_INSTALL_PREFIX=/usr/local -D BUILD_SHARED_LIBS=${BUILD_SHARED_LIBS} -D ENABLE_CXX11=ON -D OPENCV_EXTRA_MODULES_PATH=$(OPENCV_EXTRA_MODULES_PATH) -D WITH_INF_ENGINE=ON -D InferenceEngine_DIR=/usr/local/dldt/inference-engine/build -D BUILD_DOCS=OFF -D BUILD_EXAMPLES=OFF -D BUILD_TESTS=OFF -D BUILD_PERF_TESTS=OFF -D BUILD_opencv_java=NO -D BUILD_opencv_python=NO -D BUILD_opencv_python2=NO -D BUILD_opencv_python3=NO -D WITH_JASPER=OFF -D WITH_TBB=ON -DOPENCV_GENERATE_PKGCONFIG=ON -DWITH_CUDA=ON -DENABLE_FAST_MATH=1 -DCUDA_FAST_MATH=1 -DWITH_CUBLAS=1 -DCUDA_TOOLKIT_ROOT_DIR=/usr/local/cuda/ -DBUILD_opencv_cudacodec=OFF -D WITH_CUDNN=ON -D OPENCV_DNN_CUDA=ON -D CUDA_GENERATION=Auto ..
	$(MAKE) -j $(CMAKE_BUILD_PARALLEL_LEVEL)
	$(MAKE) sudo_pre_install_clean
	cd -
